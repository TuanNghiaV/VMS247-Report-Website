...
# VMS247 Dual Theme Extension

This design file is originally **light-first** and is based on a minimal Ollama-like documentation UI. For the VMS247 AI Camera report, keep the same restrained design language and add dark mode as a **neutral inverted variant**, not as a separate cyber/AI visual style.

The most important rule: **light mode is the canonical visual reference**. Dark mode must keep the same geometry, spacing, typography, borders, and restrained component vocabulary.

## Theme Strategy

Use a single component system and switch colors through CSS variables.
...
Recommended default for matching this design reference:
- `light` as the visual default when checking against `design.md`
- `dark` can be offered through the theme toggle, but it must stay minimal and neutral

## Non-negotiable Visual Rules

### Do
- Keep the page feeling like a polished technical README/report.
- Use a white paper canvas in light mode.
- Use a near-black canvas in dark mode.
- Use pill geometry for buttons, search pills, snippets, inputs, and badges.
- Use 12px radius for cards and terminal-like blocks.
- Use 1px hairline borders for cards, tables, nav dividers, and terminal cards.
- Use system sans for body, rounded/system heading fallback for titles, and monospace for code.
...
- Do not use glowing cyan/blue effects.
- Do not use cyberpunk backgrounds.
- Do not use colorful atmospheric blobs.
- Do not use cyan/blue as the primary brand color.
- Do not make dark mode look like a different product.

## Theme Tokens

Use these CSS variables as the implementation source of truth.
...
[data-theme="light"] {
  ...
  /* Compatibility aliases for existing project code */
  --bg: var(--canvas);
...
Dark mode is a restrained inversion of the light system. It should still look like a minimal documentation UI, not a neon technical dashboard.
...
## Component Mapping for VMS247

### Header / Navigation

Use the same minimal nav language as the reference:
- `height: 56px`
- white/black canvas depending on theme
- 1px bottom border using `--hairline`
- small text links
- pill buttons
- no blur-heavy glass header
- no gradient line

Recommended classes:

```tsx
<header className="fixed inset-x-0 top-0 z-50 h-14 border-b border-[var(--hairline)] bg-[var(--canvas)] text-[var(--ink)]">
```

### Primary Button

```tsx
<button className="h-9 rounded-[var(--radius-pill)] bg-[var(--primary)] px-5 text-sm font-medium text-[var(--primary-text)]">
```

### Secondary Button

```tsx
<button className="h-9 rounded-[var(--radius-pill)] border border-[var(--hairline-strong)] bg-[var(--canvas)] px-5 text-sm font-medium text-[var(--ink)]">
```

### Card

Cards are flat. No glass effect. No heavy shadow.

```tsx
<div className="rounded-[var(--radius-card)] border border-[var(--hairline)] bg-[var(--surface)] p-6 shadow-[var(--shadow-card)]">
```

### Badge / Tech Pill

```tsx
<span className="inline-flex items-center rounded-[var(--radius-pill)] border border-[var(--hairline)] bg-[var(--surface-soft)] px-3 py-1 text-xs text-[var(--body)]">
```

### Code Snippet Pill

```tsx
<div className="inline-flex rounded-[var(--radius-pill)] bg-[var(--surface-soft)] px-5 py-3 font-mono text-sm text-[var(--ink)]">
```

### Terminal Card

```tsx
<div className="rounded-[var(--radius-card)] border border-[var(--hairline)] bg-[var(--surface)] p-4 font-mono text-sm text-[var(--ink)]">
  <div className="mb-4 flex gap-1.5">
    <span className="h-3 w-3 rounded-full bg-[var(--terminal-red)]" />
    <span className="h-3 w-3 rounded-full bg-[var(--terminal-yellow)]" />
    <span className="h-3 w-3 rounded-full bg-[var(--terminal-green)]" />
  </div>
</div>
```

### Tables / Stack Matrices

Use minimal documentation tables:
- no colorful headers
- no card-like row shadows
- hairline borders only
- muted body text
- rounded outer wrapper only if needed
...
## VMS247 Content Styling Notes

Because this website is an interactive technical report, VMS247 sections may need diagrams, calculators, and stack matrices. Keep them consistent with the minimal system:

- Use pills for statuses such as `Core pilot`, `Optional`, and `Future/R&D`.
- Use neutral badges first; reserve success/warning/danger only for true semantic states.
- Do not use cyan/blue as decorative accents.
- For calculators, use flat inputs with pill radius and hairline borders.
- For flow diagrams, use simple bordered nodes and thin connector lines.
- For a single “look here” recommendation card, use `--surface-dark` in light mode or a white inverted card in dark mode, but do this sparingly.

## Agent Implementation Rules

When implementing this design in React/Tailwind:

- Prefer CSS variables over hard-coded Tailwind color classes.
- Replace `bg-gradient-*`, `from-*`, `to-*`, `shadow-2xl`, `shadow-cyan-*`, `border-cyan-*`, `text-cyan-*`, and heavy `backdrop-blur-*` patterns.
- Use `bg-[var(--canvas)]`, `bg-[var(--surface)]`, `text-[var(--ink)]`, `text-[var(--body)]`, and `border-[var(--hairline)]`.
- Keep motion subtle. The reference design relies on spacing and restraint, not animation.
- Keep full-page slide behavior independent from visual styling.

## Recommended `src/styles/theme.css`

The CSS blocks under **Theme Tokens** can be copied directly into `src/styles/theme.css`.
